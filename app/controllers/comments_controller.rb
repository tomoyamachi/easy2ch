...
class CommentsController < ApplicationController
  ...
  def show_thread
    @category = params[:c]
    get_category
    @comments = Comment.where("category = ?",@category).order('created_at DESC').all
  end
  ...
  def create
    ...
    if @comment.save
      redirect_to(:action =>"show_thread", :c => params[:category], :notice => '書き込みに成功しました。')
    else
      redirect_to(:action =>"show_thread", :c => params[:category], :notice => '書き込みに失敗しました。')
    end
  end
